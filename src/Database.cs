...
public class Database
{
        ...
        try
        {
            ...
            var statements = new[]
            {
                ...
                @"CREATE TABLE IF NOT EXISTS player_cs2_setting_configs (
                    ...
                    updated_at DATETIME   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    PRIMARY KEY (steamid, category, `key`),
    ...
    public async Task UpsertPlayerAsync(string steamId, string name, List<PlayerSetting> configs)
    {
        try
        {
            ...
            {
                cmd.CommandText = @"
                    INSERT INTO player_cs2_setting_configs (steamid, category, `key`, value, updated_at)
                ...
                foreach (var setting in configs)
                {
                    ...
                    pCat.Value = setting.Category.ToString();
                    pKey.Value = setting.Key;
                    pVal.Value = setting.Value;
                    await cmd.ExecuteNonQueryAsync();
                }
            }
